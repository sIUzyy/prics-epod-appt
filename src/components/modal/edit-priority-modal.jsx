...
export default function EditPriorityModal({
  ...
  const { trackingNo, priority } = selectedTrackingNo || {};

  console.log(selectedTrackingNo);
  ...
  const handleUpdateActivity = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_ENDPOINT}/api/shipment/${trackingNo}/update-priority`,
        updatedActivityData
      );

      if (response.status === 200) {
        toast.success(
          `Successfully update the priority of tracking no. ${trackingNo}`,
          {
            style: {
      ...
      console.log(error);
      toast.error(
        `Failed to update the priority of tracking no. ${trackingNo}. Please try again later.`,
        {
          style: {
