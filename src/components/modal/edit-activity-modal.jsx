...
export default function EditActivityModal({ open, setOpen, selectedActivity }) {
  const { _id, activityName, description } = selectedActivity || {};

  console.log(selectedActivity);

  // state for fields
  const [editActivityName, setEditActivityName] = useState(activityName);
  ...
  const handleUpdateActivity = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_ENDPOINT}/api/activity/${_id}/update-activity`,
        updatedActivityData
      );
